...
df <- NYC_Jobs %>% select(-`Posting Type`,-`Business Title`,-`Title Classification`,-`Title Code No`,-Level,-`Job Description`,-`Minimum Qual Requirements`,-`Preferred Skills`,-`Additional Information`,-`To Apply`, -`Work Location 1`,-`Recruitment Contact`,-`Residency Requirement`,-`Post Until`, -`Posting Updated`, -`Process Date`, -`Hours/Shift`)
...
df <- df %>% mutate(title = case_when(
  ...
  `Civil Service Title` %>% str_detect("CLAIM SPECIALIST|CLERICAL ASSOCIATE") == 1 ~ "Claim Assoviate",
...
title_category <- title_category %>% select(-`Salary Range From`, -`Salary Frequency`)
```
...
save(na_drop_jobs, file = "../../data/na_drop.RData")
```
